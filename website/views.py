...
def writing():
    ...
    if request.method == "POST":
        ...
        sleutelwoord = request.form.get("sw1") #,request.form.get("sw2"),request.form.get("sw3")]
        
        #input checken op correcte invoer
        if not onderwerp or not lengte or not schrijfdoel:
        ...
        elif not sleutelwoord.isalpha() and sleutelwoord: #als er wel iets ingevuld is en het niet alphabetic is
            flash("Sleutelwoord moet een woord zijn")
        elif len(sleutelwoord) <= 2:
            flash("Sleutelword moet een volledig woord zijn")
            return render_template("writing.html")
            #nog meer inputchecks maken
 
        #correcte invoer
        output = get_gpt3_response(("Schrijf een academische tekst met als doel {} over {}. De tekst moet het sleutelwoord {} bevatten. Jouw antwoord moet ongeveer {} woorden lang zijn.".format(schrijfdoel, onderwerp, sleutelwoord, lengte)), 3*int(lengte)) # Het moet de volgende sleutelwoorden bevatten: {}, sleutelwoorden
        ...
        output = output[2:]
        return render_template("resultaat.html", output=output)
